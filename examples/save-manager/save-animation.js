...
state.preload = function () {
	...
	// this.addImage('object', '../assets/object.png');
	// this.addSpriteSheet('sprite', '../assets/sprite.png', 150, 117);
	this.addSpriteSheet( 'sprite', './assets/img/anime-monsters/griffon-sheet.png', 150, 117 );
...
state.create = function () {
    ...
    this.spriteOne = new Kiwi.GameObjects.Sprite(this, this.textures.sprite, 50, 100, true);
    this.spriteTwo = new Kiwi.GameObjects.Sprite(this, this.textures.sprite, 300, 100, true);
    this.spriteThree = new Kiwi.GameObjects.Sprite(this, this.textures.sprite, 550, 100, true);
    ...
    if (this.game.saveManager.localStorage.exists('first')) {
        this.spriteOne.cellIndex = this.game.saveManager.localStorage.getData('first');
    }

    //Does the second one exist?
    if (this.game.saveManager.localStorage.exists('second')) {
        this.spriteTwo.cellIndex = this.game.saveManager.localStorage.getData('second');
    }

    //Does the third one exist?
    if (this.game.saveManager.localStorage.exists('third')) {
        this.spriteThree.cellIndex = this.game.saveManager.localStorage.getData('third');
    }
...
state.increaseOne = function () {
    ...
    this.game.saveManager.localStorage.add('first', this.spriteOne.cellIndex);
};
...
state.increaseTwo = function () {
    ...
    this.game.saveManager.localStorage.add('second', this.spriteTwo.cellIndex);
};
...
state.increaseThree = function () {
    ...
    this.game.saveManager.localStorage.add('third', this.spriteThree.cellIndex);
};
...
var game = new Kiwi.Game('game-container', 'Save and Load Animation', state, gameOptions);
